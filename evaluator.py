...
def stepwise_error(y_true, y_pred, n_steps, rate=0.2):    
    ...
    for i in range(y_true.shape[0]):
        for t in range(n_steps):
            ...
            with tf.device('/cpu:0'):
                t_true = tf.math.top_k(y_true[i,t,:], k=cnt)  # round(n_districts * rate)
                t_true = set(t_true.indices.numpy()[[di for di, val in enumerate(t_true.values.numpy()) if val > 0]])
                t_pred = set(tf.math.top_k(y_pred[i,t,:], k=cnt).indices.numpy())  # round(n_districts * rate)
